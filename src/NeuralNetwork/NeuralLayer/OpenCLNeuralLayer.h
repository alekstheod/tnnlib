...
namespace nn {
    ...
    namespace detail {
        ...
        template < class Internal > class OpenCLNeuralLayer {
            public:
            ...
            template < template < class > class NewType > using wrap = OpenCLNeuralLayer< typename Internal::template wrap< NewType > >;

            template < unsigned int inputs > using resize = OpenCLNeuralLayer< typename Internal::template resize< inputs > >;

            template < typename VarType > using use = OpenCLNeuralLayer< typename Internal::template use< VarType > >;
            ...
            void calculate () {
                ...
                try {
                    ...
                    for (int offset = 0; offset < CONST_NEURONS_NUMBER; ++offset) {
                        ...
                        queue.enqueueNDRangeKernel (m_kernel, cl::NDRange (offset), cl::NDRange (rangeSize));
                    }
                    ...
                    cl_int err;
                    cl::STRING_CLASS buildlog = m_program.getBuildInfo< CL_PROGRAM_BUILD_LOG > (devices[0], &err);
                    std::cout << "Building error! Log: " << buildlog << std::endl;
                }
            }
            ...
            public:
            OpenCLNeuralLayer () : m_context (createContext ()), m_program (createProgram (m_context)), m_kernel (m_program, "dot_product") {
            }
    ...
    template < template < template < class > class, class, std::size_t, int > class NeuronType, template < class > class ActivationFunctionType, std::size_t size, std::size_t inputsNumber = 2, int scaleFactor = 1 >
