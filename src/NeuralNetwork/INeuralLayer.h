...
namespace nn {
    ...
    template < typename NeuralLayerType > class INeuralLayer {
        public:
        ...
        public:
        template < typename... Args > INeuralLayer (Args... args) : m_neuralLayer (args...) {
        }
        ...
        /**
         * @brief Will set the input value by using the given input id. If the given
         * input is is invalid value will not be set.
    ...
    NeuralLayer createLayer (const typename NeuralLayer::template Memento< typename NeuralLayer::NeuronMemento, NeuralLayer::CONST_NEURONS_NUMBER >& memento) {
        NeuralLayer layer;
