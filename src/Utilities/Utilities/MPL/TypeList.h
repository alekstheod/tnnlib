...
namespace utils {
    ...
    template < class U, class T > struct TypeList {
        typedef U Head;
        typedef T Tail;
    };
    ...
    template <> struct Length< TypeList< NullType, NullType > > {
        enum { value = 0 };
    };
    ...
    template < class Head, class Tail > struct Reverse< TypeList< Head, Tail > > { typedef typename Append< typename Reverse< Tail >::Result, Head >::Result Result; };

    template < unsigned int position, class TList > struct TypeAtPos { typedef typename TypeAtPos< position - 1, typename TList::Tail >::Result Result; };

    template < class Head, class Tail > struct TypeAtPos< 0, TypeList< Head, Tail > > { typedef Head Result; };

    template < unsigned int pos > struct TypeAtPos< pos, NullType > { typedef NullType Result; };

    template < bool value, typename T1, typename T2 > struct if_;

    template < typename T1, typename T2 > struct if_< true, T1, T2 > { typedef T1 Result; };

    template < typename T1, typename T2 > struct if_< false, T1, T2 > { typedef T2 Result; };

    template < typename T, typename TList > struct IsInList { typedef typename IsInList< T, typename TList::Tail >::Result Result; };

    template < typename T > struct IsInList< T, NullType > { typedef False Result; };

    template < typename T, typename U > struct IsInList< T, TypeList< T, U > > { typedef True Result; };
}
