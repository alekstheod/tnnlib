...
typedef nn::Perceptron< VarType, nn::NeuralLayer< nn::Neuron, nn::SigmoidFunction, 10, inputsNumber, 1000 >, nn::NeuralLayer< nn::Neuron, nn::SigmoidFunction, 80, 10, 1000 >, nn::NeuralLayer< nn::Neuron, nn::SoftmaxFunction, 10, 1000 > > Perceptron;

typedef nn::Perceptron< VarType, nn::NeuralLayer< nn::Neuron, nn::SigmoidFunction, 20, inputsNumber, 1000 >, nn::NeuralLayer< nn::Neuron, nn::SoftmaxFunction, inputsNumber, 80, 1000 > > AutoEncoder;
...
template < typename SrcView, typename DstView > void convert_color (const SrcView& src, const DstView& dst) {
...
template < typename Iterator > void readImage (std::string fileName, Iterator out) {
    using namespace boost::gil;
    using namespace nn;
...
template < typename Perc > void save (const Perc& perc, std::string name) {
...
template < typename Files > AutoEncoder calculateAutoEncoder (Files files) {
    ...
    for (auto image : files) {
        if (!boost::filesystem::is_directory (image)) {
            try {
                ...
                std::copy (std::get< 0 > (proto).begin (), std::get< 0 > (proto).end (), std::get< 1 > (proto).begin ());
    ...
    static AutoEncoder enc = autoEncAlgo.calculate (prototypes.begin (), prototypes.end (), [](VarType error) {
        counter++;
        if (counter > 0) {
            counter = 0;
...
void calculateWeights (std::string imagesPath) {
    ...
    static Perceptron perceptron = algorithm.calculate (prototypes.begin (), prototypes.end (), errorFunc);
