...
namespace {
    ...
    SCENARIO("Perceptron input distribution",
        ...
        GIVEN("A perceptron with 3 neurons, 2 features each") {
            ...
            auto& inputLayer = std::get< 0 >(perceptron.layers());
            for(auto& neuron : inputLayer) {
                neuron.setBias(0.0f);
                neuron.setWeight(0, 1.0f);
                neuron.setWeight(1, 0.0f);
            }
            auto& hiddenLayer = std::get< 1 >(perceptron.layers());
            hiddenLayer[0].setBias(0.0f);
            for(int i = 0; i < 3; ++i) {
                hiddenLayer[0].setWeight(i, 1.0f);
            }
